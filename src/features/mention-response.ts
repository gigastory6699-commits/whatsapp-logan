...
import { shouldRouteToAgent, buildVideoPrompt, extractVideoDescription, buildLandingPagePrompt, extractLandingPageDescription, isFreeChatGroup, extractImageDescription } from '../utils/agent-triggers';
import { sendVideo, sendImage, sendMedia, detectMediaType } from '../utils/media-sender';
...
async function handleImageRequest(
  ...
  const cleanPrompt = extractImageDescription(message);
  console.log(`[IMAGE] Processing image request from ${senderName}: "${cleanPrompt}"`);

  const sock = getSocket();

  // STEP 0: Refresh group session FIRST (required for reactions to work)
  if (sock && groupId.endsWith('@g.us')) {
    try {
      console.log('[IMAGE] Refreshing group session...');
      await sock.groupMetadata(groupId);
    } catch (metaErr) {
      console.log('[IMAGE] Group metadata fetch failed (continuing anyway):', metaErr);
    }
  }
  ...
  if (sock && messageKey) {
    try {
      ...
      console.error('[IMAGE] React failed:', e);
    }
  }

  try {
    // STEP 2: Call generateImage service
    ...
    console.log(`[IMAGE] Image generated successfully: ${result.filePath}`);

    // STEP 3: Send image with caption
    const caption = `🎨 *התמונה שלך מוכנה!* (Your image is ready!)\n\n*תיאור / Prompt:* ${cleanPrompt}\n\n*Expanded Prompt:* _${result.expandedPrompt || cleanPrompt}_\n\n_Generated by Logan using Flux_`;

    const sendResult = await sendImage(groupId, result.filePath, caption, senderNumber);

    if (!sendResult.success) {
      throw new Error(sendResult.error || 'Failed to send image');
    }

    // STEP 4: React with ✅ emoji
    if (sock && messageKey) {
      try {
        await sock.sendMessage(groupId, { react: { text: '✅', key: messageKey } });
      } catch (e) {}
    ...
    // STEP 5: Schedule file cleanup after 1 minute
    const filePath = result.filePath;
    setTimeout(() => {
      try {
        if (fs.existsSync(filePath)) {
          fs.unlinkSync(filePath);
          console.log(`[IMAGE] Cleaned up temp file: ${filePath}`);
        }
      } catch (cleanupErr) {
        console.error(`[IMAGE] Failed to cleanup temp file ${filePath}:`, cleanupErr);
      }
    }, 60000);

    return true;
  } catch (error) {
    console.error('[IMAGE] Error in handleImageRequest:', error);
    ...
    const errorMsg = `❌ אירעה שגיאה ביצירת התמונה.\n\nפרטי השגיאה: ${error instanceof Error ? error.message : String(error)}`;
  ...
  try {
    ...
    if (isCopilotAgentEnabled()) {
      ...
      if (triggerResult.shouldUseAgent) {
        console.log(`[MENTION] Routing to Agent/Service (trigger: ${triggerResult.triggerType})`);

        let handled = false;

        // Handle image requests directly
        if (triggerResult.isImageRequest) {
          handled = await handleImageRequest(
            groupId,
            senderNumber,
            senderName,
            message,
        ...
        } else if (triggerResult.isDirectCommand && triggerResult.extractedCommand) {
        ...
        if (triggerResult.isDirectCommand || triggerResult.isImageRequest) {
          return; // Don't fallback for direct commands or image requests
        }
...
export async function handleVoiceMention(
  ...
  try {
    ...
    if (isCopilotAgentEnabled()) {
      ...
      if (triggerResult.shouldUseAgent) {
        console.log(`[VOICE] Routing to Agent/Service (trigger: ${triggerResult.triggerType})`);
    ...
    try {
      ...
      // STEP 5: Send response (try voice first, fallback to text)
      ...
      if (sock) {
        ...
        try {
          // Set presence to recording...
          await sock.sendPresenceUpdate('recording', recipientJid);
          console.log(`[VOICE] Synthesizing speech for: "${response.substring(0, 60)}..."`);
          
          const audioBuffer = await textToSpeech(response);
          if (audioBuffer && audioBuffer.length > 0) {
            console.log(`[VOICE] Sending audio voice note (${audioBuffer.length} bytes)...`);
            ...
            console.log(`[VOICE] Voice response sent successfully to ${senderName}`);
            voiceSent = true;
            recordResponse(senderNumber);
            ...
            console.warn('[VOICE] Failed to generate TTS audio buffer, falling back to text response');
          }
        } catch (ttsErr) {
          console.error('[VOICE] Error during voice synthesis/sending:', ttsErr);
        } finally {
          try {
            await sock.sendPresenceUpdate('paused', recipientJid);
          } catch {}
      ...
      if (!voiceSent) {
        // Fallback: Format response with transcription reference and send as text
        const formattedResponse = formatVoiceResponse(response, transcription);
        console.log('[VOICE] Fallback: Sending text response...');
        const sent = await sendResponse(groupId, formattedResponse, senderNumber, messageKey);

        if (sent) {
          console.log(`[VOICE] Responded to ${senderName} (text fallback)`);
          recordResponse(senderNumber);
        } else {
          console.error('[VOICE] Failed to send text fallback response');
        }
      }
    } finally {
      // Ensure typing is stopped even if an error occurs
